...
O FULL RAY-X é a fotografia profunda. Antes da primeira alteração, precisamos de uma fotografia curta e repetível que será usada **antes e depois de toda Change**.

Ela também captura SHA-256 diretamente dos `FragmentPath` e `DropInPaths` crus das nove units, dado que não devemos inferir hash de unit a partir de `systemctl cat`.
...
Candidate foi validado com `bash -n` antes da publicação local; SHA-256 da versão preparada: `8f965d5a00c72470b17027ec73ff230e5f7b1dbc18b36197cb49077d14387c78`.

O hash executado no host será registrado novamente e deve corresponder à versão obtida do GitHub.
...
- HLS freshness quando houver manifest;
...
- raw hash dos drop-ins carregados;
...
- se houver commit novo da Engenharia TV/Core/Rádio, revisar antes de executar.
...
4. unit/drop-in hashes reais registrados;
5. hashes estáticos confrontados com FULL RAY-X;
6. diferenças entre FULL RAY-X e snapshot atual explicadas;
7. `critical-artifacts-baseline.yaml` atualizado e status convertido para locked;
8. `CHANGE_QUEUE.md` atualizado;
9. `main` relido após a documentação.
