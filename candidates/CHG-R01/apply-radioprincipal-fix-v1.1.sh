...
# DOES NOT run systemctl daemon-reload, MediaMTX/NGINX restart, or touch other stations.
set -Eeuo pipefail
IFS=$'\n\t'
...
for c in systemctl sha256sum ffprobe ffmpeg find sort grep awk jq curl timeout runuser install stat cp mv tar date git wc tr mktemp seq; do need "$c"; done
...
echo "CHG-R01 Radio Principal transactional apply v1.1"
echo "UTC=$TS"
echo "REPO=$REPO"
echo "OUT=$OUT"
...
ELIGIBLE="$(find "$READY" -type f \( -iname '*.mp3' -o -iname '*.m4a' -o -iname '*.aac' \) ! -iname '*teste*' ! -iname '*test*' -printf 'x' | wc -c | tr -d ' ')"
...
GEN_UID="$(stat -c '%u' "$GEN")"; GEN_GID="$(stat -c '%g' "$GEN")"; GEN_MODE="$(stat -c '%a' "$GEN")"
...
for _ in $(seq 1 20); do [[ "$(systemctl is-active "$UNIT" || true)" == "active" ]] && break; sleep 1; done
...
timeout 15 ffprobe -v error -rtsp_transport tcp -select_streams a:0 -show_entries stream=codec_name,sample_rate,channels -of default=noprint_wrappers=1 rtsp://127.0.0.1:8554/radioprincipal | tee "$OUT/rtsp.post.txt"
...
IMPOSSIBLE="$(grep -ci 'Impossible to open' "$OUT/journal.post.txt" || true)"; NO_READY="$(grep -ci 'NO_READY_MEDIA' "$OUT/journal.post.txt" || true)"
echo "impossible_to_open_post=$IMPOSSIBLE"; echo "no_ready_media_post=$NO_READY"
[[ "$IMPOSSIBLE" -eq 0 ]] || fail "IMPOSSIBLE_TO_OPEN_POST_RESTART"
[[ "$NO_READY" -eq 0 ]] || fail "NO_READY_MEDIA_POST_RESTART"

echo "=== L. PROVE NO OTHER PROCESS/PLAYLIST CHANGED ==="
printf 'unit\tpid\n' > "$OUT/pids.post.tsv"
for u in tps-radioprincipal-playout.service tps-radiopop-playout.service tps-radiorock-playout.service tps-radioclassicas-playout.service tps-radiocountry-playout.service tps-tvkids-playout.service tps-tvteens-playout.service tps-tvviva-playout.service tps-tvmaisjovem-playout.service tps-mediamtx.service nginx.service; do
  printf '%s\t%s\n' "$u" "$(systemctl show "$u" -p MainPID --value)" >> "$OUT/pids.post.tsv"
done
awk -F '\t' 'NR==FNR{pre[$1]=$2;next} $1!="tps-radioprincipal-playout.service" && pre[$1]!=$2 {print "UNEXPECTED_PID_CHANGE=" $1 " pre=" pre[$1] " post=" $2; bad=1} END{exit bad}' "$OUT/pids.pre.tsv" "$OUT/pids.post.tsv" || fail "OTHER_PROCESS_PID_CHANGED"
...
awk -F '\t' 'NR==FNR{pre[$1]=$2;next} $1!="radioprincipal" && pre[$1]!=$2 {print "UNEXPECTED_PLAYLIST_CHANGE=" $1; bad=1} END{exit bad}' "$OUT/playlists.pre.tsv" "$OUT/playlists.post.tsv" || fail "OTHER_PLAYLIST_CHANGED"
echo "other_playlists_unchanged=PASS"

echo "=== M. FINAL IDENTITIES / SHAREABLE ==="
