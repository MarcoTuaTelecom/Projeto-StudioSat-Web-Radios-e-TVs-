...
for ch in "${RADIOS[@]}"; do
  ...
  set -e
  printf '%s\n' "$PROBE" >"$D/ffprobe.txt"
  RTMP_OK=NO; [ "$RTMP_RC" -eq 0 ] && [ -n "$PROBE" ] && RTMP_OK=YES
  HLSL=NO; hls_ready "$HLS_LOCAL" && HLSL=YES
  HLSP=NO; hls_ready "$HLS_PUBLIC" && HLSP=YES
...
FAIL=0
for ch in "${RADIOS[@]}"; do
  R=NO; media_ready "rtmp://127.0.0.1:1935/$ch" && R=YES
  L=NO; hls_ready "http://127.0.0.1:8888/$ch/index.m3u8" && L=YES
  P=NO; hls_ready "$PUBLIC_HOST/$ch/index.m3u8" && P=YES
...
echo "BUNDLE=${OUT}.tar.gz" | tee -a "$REPORT"
