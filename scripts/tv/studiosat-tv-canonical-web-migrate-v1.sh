#!/usr/bin/env bash
# Nome: studiosat-tv-canonical-web-migrate-v1.sh
# Versão: 1.0
# Owner: TV + Core
...
RADIO_HOST_RE='(^|[[:space:]])(radio|radioprincipal|radiopop|radiorock|radioclassicas|radiocountry)(\.studiosatweb\.com\.br)?([[:space:];]|$)'
MUTATED=0
INDEX_EXISTED=0
NGINX_CANON_EXISTED=0
OWNERS_TAR_CREATED=0

have(){ command -v "$1" >/dev/null 2>&1; }
die(){ echo "FATAL=$*" >&2; exit 1; }
sha(){ sha256sum "$1" | awk '{print $1}'; }
for c in git systemctl curl jq nginx sha256sum awk grep sed flock install cp mv rm mkdir find sort xargs diff tar date timeout ffprobe python3 basename dirname; do have "$c" || die "MISSING_TOOL:$c"; done
...
# --- Immutable production baseline: Radio + MediaMTX ---
...
# --- Discover ACTIVE TV owners from nginx -T, not from assumed filenames ---
nginx -t > "$OUT/nginx-pre-test.txt" 2>&1 || die NGINX_PRETEST_FAIL
nginx -T > "$OUT/nginx-T.pre.txt" 2>&1 || die NGINX_T_PRE_FAIL
python3 - "$OUT/nginx-T.pre.txt" "$OUT/tv-owner-files.txt" <<'PY'
import re,sys
...
for line in open(src,encoding='utf-8',errors='replace'):
    ...
    if cur and 'server_name' in line:
        if any(h in line for h in tv): owners.add(cur)
open(out,'w',encoding='utf-8').write(''.join(p+'\n' for p in sorted(owners)))
PY

# Candidate destination is permitted; every other owner must be TV-only.
: > "$OUT/retire-files.txt"
while IFS= read -r f; do
  [[ -n "$f" ]] || continue
  [[ -e "$f" || -L "$f" ]] || die "ACTIVE_OWNER_PATH_MISSING:$f"
  if [[ "$f" == "$NGINX_DST" ]]; then
    continue
  fi
  if grep -Eqi "$RADIO_HOST_RE" "$f"; then die "TV_OWNER_MIXES_RADIO:$f"; fi
  python3 - "$f" <<'PY'
import re,sys
p=sys.argv[1]
allowed={
...
names=[]
for m in re.finditer(r'\bserver_name\s+([^;]+);',text,re.S):
    names.extend(m.group(1).split())
foreign=[n for n in names if n not in allowed and n not in {'_','""'}]
if foreign:
    print('FOREIGN_SERVER_NAMES='+' '.join(sorted(set(foreign))))
    raise SystemExit(1)
PY
  rc=$?
  [[ $rc -eq 0 ]] || die "TV_OWNER_HAS_NON_TV_NAMES:$f"
...
# Backup exact active owners before mutation, including symlinks.
mapfile -t RETIRE < "$OUT/retire-files.txt"
BACKUP_PATHS=()
for f in "${RETIRE[@]}"; do [[ -n "$f" ]] && BACKUP_PATHS+=("$f"); done
[[ -e "$NGINX_DST" || -L "$NGINX_DST" ]] && { NGINX_CANON_EXISTED=1; BACKUP_PATHS+=("$NGINX_DST"); }
if ((${#BACKUP_PATHS[@]})); then
  tar -czPf "$BACKUP/nginx-tv-owners.before.tar.gz" "${BACKUP_PATHS[@]}"
  OWNERS_TAR_CREATED=1
fi
[[ -f "$INDEX" ]] && { INDEX_EXISTED=1; cp -a "$INDEX" "$BACKUP/index.html.before"; }

# Validate candidate syntax in isolation by temporarily including it only through nginx full install later.
# The existing config remains untouched until all classification gates above pass.
echo TV_OWNER_CLASSIFICATION=PASS

rollback(){
  set +e
  echo '=== ROLLBACK CHG-TV-CANON-001 ==='
  rm -f "$NGINX_DST"
  for f in "${RETIRE[@]}"; do [[ -n "$f" ]] && rm -f "$f"; done
  if (( OWNERS_TAR_CREATED == 1 )); then tar -xzPf "$BACKUP/nginx-tv-owners.before.tar.gz"; fi
  if (( INDEX_EXISTED == 1 )); then install -d -o www-data -g www-data -m 0755 "$WEBROOT"; cp -a "$BACKUP/index.html.before" "$INDEX"; else rm -f "$INDEX"; fi
  nginx -t >/dev/null 2>&1 && systemctl reload nginx >/dev/null 2>&1 || true
  echo ROLLBACK=DONE
}
trap 'rc=$?; if (( rc != 0 && MUTATED == 1 )); then rollback; fi; exit $rc' EXIT

# --- Mutation: retire ALL old TV-only owners, then install one canonical owner ---
MUTATED=1
for f in "${RETIRE[@]}"; do [[ -n "$f" ]] && rm -f "$f"; done
...
# Exactly one active FILE may own any TV hostname: the canonical file.
python3 - "$OUT/nginx-T.post.txt" "$NGINX_DST" <<'PY'
import re,sys
...
if bad:
    for h,v in bad.items(): print(h,':',','.join(v))
    raise SystemExit(1)
PY
[[ $? -eq 0 ]] || die TV_HOST_OWNERSHIP_NOT_CANONICAL

systemctl reload nginx
sleep 2

# --- Verify locally through SNI, bypassing DNS/cache ---
...
: > "$OUT/tv-public-local.tsv"
for h in "${TV_HOSTS[@]}"; do
  st="${STATION_BY_HOST[$h]}"; hdr="$OUT/$h.headers"; body="$OUT/$h.root"
  ...
  printf '%s\t%s\tROOT_PASS\n' "$h" "$st" >> "$OUT/tv-public-local.tsv"
done

# Stream proof is factual per station. Web migration does not fake an offline stream as healthy.
...
for st in "${TVS[@]}"; do
  ready="$(jq -r --arg s "$st" '.items[]?|select(.name==$s)|.ready' <<<"$paths" | head -1)"; [[ -n "$ready" ]] || ready=false
  if [[ "$ready" == true ]]; then
    host=""
    case "$st" in tvkids) host=tvkidsweb.studiosatweb.com.br;; tvteens) host=tvteens.studiosatweb.com.br;; tvviva) host=tvviva.studiosatweb.com.br;; tvmaisjovem) host=tvmaisjovem.studiosatweb.com.br;; esac
    hls="$OUT/$st.m3u8"; hc="$(curl -kLsS --resolve "$host:443:127.0.0.1" --max-time 15 -o "$hls" -w '%{http_code}' "https://$host/$st/index.m3u8" || true)"
    [[ "$hc" == 200 ]] && grep -q '^#EXTM3U' "$hls" || die "READY_STREAM_NOT_PUBLIC:$st:$hc"
    printf '%s\tREADY\tPUBLIC_HLS_PASS\n' "$st" >> "$OUT/tv-stream-state.tsv"
  else
    printf '%s\tNOT_READY\tWEB_ONLY_OK\n' "$st" >> "$OUT/tv-stream-state.tsv"
  fi
done

# --- Invariants after graceful NGINX reload ---
...
for d in /var/www/studiosat-radio-player /var/www/studiosat-radio-portal /var/www/studiosat-radio/current; do
  key="$(basename "$d")"; find "$d" -type f -print0 | sort -z | xargs -0 sha256sum > "$OUT/$key.post.sha256"; diff -u "$OUT/$key.pre.sha256" "$OUT/$key.post.sha256" > "$OUT/$key.diff" || die "RADIO_WEBROOT_CHANGED:$d"
done
