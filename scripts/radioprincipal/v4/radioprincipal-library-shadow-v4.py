...
BASE=Path("/var/lib/studiosat/radio-principal-v4")
...
ROOTS=[
    Path("/srv/studiosat/radio-principal/grade/manha"),
    Path("/srv/studiosat/radio-principal/grade/tarde"),
    Path("/srv/studiosat/radio-principal/grade/noite"),
    Path("/srv/studiosat/radio-principal/elementos"),
    Path("/srv/tpsmedia/repository/channels/radioprincipal/mirror-store"),
    Path("/srv/tpsmedia/repository/channels/radioprincipal/ready"),
]
...
def track_attr(t,key):
    if key in t.attrib:
        return t.attrib.get(key,"")
    for child in list(t):
        if child.tag.casefold()=="tag" and key in child.attrib:
...
def scan_library():
    ...
    idx={
        "schema":"studiosat.library-index.v1",
...
def resolve_item(item,maps):
    ...
    if c:
        item.update({
            "available":True,
            "local_path":c[0]["path"],
            "resolution":"local-library",
            "candidates":len(c),
        })
    else:
        item.update({"available":False,"local_path":"","resolution":"missing","candidates":0})
    return item
...
def build_queue(index):
    ...
    resolved=[resolve_item(dict(t),maps) for t in tracks]
...
class Shadow:
    ...
    def refresh(self,force=False):
        now=time.monotonic()
        if force or now-self.last_index_refresh>60:
            self.index=scan_library()
            self.last_index_refresh=now
        if force or now-self.last_queue_refresh>2:
    ...
    def write_state(self):
        ...
        atomic_json(STATE,{
            "schema":"studiosat.shadow-v4.state.v1",
