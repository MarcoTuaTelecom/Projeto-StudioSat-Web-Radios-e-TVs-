...
SYNC=Path(os.environ.get("STUDIOSAT_V42_SYNC","/var/lib/studiosat/radio-v2/radioboss-sync/radioprincipal/current"))
PLAYLIST=SYNC/"playlist.json"
PLAYBACK=SYNC/"playback.json"
SCHEDULE=SYNC/"schedule.json"
MANIFEST=SYNC/"librarymanifest.json"

BASE=Path(os.environ.get("STUDIOSAT_V42_BASE","/var/lib/studiosat/radio-principal-v42"))
...
CHUNK=3840
SILENCE=b"\x00"*CHUNK
CONTROL_FRESH_SEC=float(os.environ.get("STUDIOSAT_V42_CONTROL_FRESH_SEC","30"))
...
def scan_library():
    ...
    idx={
        "schema":"studiosat.library-index.v1.2",
...
def resolve_item(item,maps,index,mmanifest):
    ...
    if c:
        method="manifest-sha" if any((e.get("sha_from_filename") or "") for e in c) and not (bkey in exact) else "local-exact"
        item.update({
            "available":True,
            "local_path":c[0]["path"],
...
class Shadow:
    ...
    def write_state(self):
        ...
        atomic_json(STATE,{
            "schema":"studiosat.shadow-v4.2.state.v1",
