...
SYNC=Path(os.environ.get("STUDIOSAT_STABLE_SYNC","/var/lib/studiosat/radio-v2/radioboss-sync/radioprincipal/current"))
PLAYLIST=SYNC/"playlist.json"
SYNC_PLAYBACK=SYNC/"playback.json"
CONTROL_PLAYBACK=Path(os.environ.get(
    "STUDIOSAT_STABLE_CONTROL_PLAYBACK",
    "/run/studiosat-radioprincipal-v8-control/playback.json"
))
SCHEDULE=SYNC/"schedule.json"
MANIFEST=SYNC/"librarymanifest.json"

BASE=Path(os.environ.get("STUDIOSAT_STABLE_BASE","/var/lib/studiosat/radio-principal-stable"))
INDEX=BASE/"library-index.json"
QUEUE=BASE/"effective-queue.json"
STATE=BASE/"state.json"
RTMP=os.environ.get("STUDIOSAT_STABLE_RTMP","rtmp://127.0.0.1:1935/radioprincipal")

ROOTS=[
    Path(x) for x in os.environ.get(
        "STUDIOSAT_STABLE_ROOTS",
...
CHUNK=3840
SILENCE=b"\x00"*CHUNK
CONTROL_FRESH_SEC=float(os.environ.get("STUDIOSAT_STABLE_CONTROL_FRESH_SEC","15"))
MEDIA_DB=Path(os.environ.get("STUDIOSAT_STABLE_MEDIA_DB","/var/lib/studiosat/radio-v2/media-transfer/index.sqlite3"))
...
class Playout:
    ...
    def refresh(self,force=False):
        now=time.monotonic()
        if force or now-self.last_index_refresh>120:
            self.index=scan_library()
            self.last_index_refresh=now
        if force or now-self.last_queue_refresh>5:
    ...
    def desired(self):
        ...
        # Stale control is used only once to choose an approximate starting point.
        # After that, stay on the local authoritative RadioBOSS queue and never
        # re-seek every refresh. This prevents audible skips and repeated jumps.
        if self.stale_locked and self.current_index is not None and q.get("queue"):
            if 0<=self.current_index<len(q["queue"]):
                self.mode="autonomous-stable"
        ...
        self.mode="autonomous-stable"
        self.stale_locked=True
        if self.current_index is not None and 0<=self.current_index<len(q.get("queue") or []):
            return q["queue"][self.current_index],None,age
    ...
    def ensure_decoder(self):
        ...
        if not need and self.mode=="hot-sync" and seek is not None:
            ...
            # Large correction only. Small drift must never create audible jumps.
            if abs(local_pos-seek)>10.0:
                need=True
        if need:
    ...
    def write_state(self):
        ...
        atomic_json(STATE,{
            "schema":"studiosat.radioprincipal.stable-shadow.v1",
    ...
    def run(self):
        ...
        while RUN:
            ...
            if self.dec and self.dec.poll() is not None:
                if self.mode!="hot-sync" and self.current_index is not None:
