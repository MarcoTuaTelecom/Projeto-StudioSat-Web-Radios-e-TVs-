...
SYNC=Path(os.environ.get("STUDIOSAT_V51_SYNC","/var/lib/studiosat/radio-v2/radioboss-sync/radioprincipal/current"))
PLAYLIST=SYNC/"playlist.json"
SYNC_PLAYBACK=SYNC/"playback.json"
CONTROL_PLAYBACK=Path(os.environ.get(
    "STUDIOSAT_V51_CONTROL_PLAYBACK",
    "/run/studiosat-radioprincipal-v8-control/playback.json"
))
SCHEDULE=SYNC/"schedule.json"
MANIFEST=SYNC/"librarymanifest.json"

BASE=Path(os.environ.get("STUDIOSAT_V51_BASE","/var/lib/studiosat/radio-principal-v51"))
INDEX=BASE/"library-index.json"
QUEUE=BASE/"effective-queue.json"
STATE=BASE/"state.json"
RTMP=os.environ.get("STUDIOSAT_V51_RTMP","rtmp://127.0.0.1:1935/radioprincipal")
...
CHUNK=3840
SILENCE=b"\x00"*CHUNK
CONTROL_FRESH_SEC=float(os.environ.get("STUDIOSAT_V51_CONTROL_FRESH_SEC","15"))
MEDIA_DB=Path(os.environ.get("STUDIOSAT_V51_MEDIA_DB","/var/lib/studiosat/radio-v2/media-transfer/index.sqlite3"))
...
def scan_library():
    ...
    idx={
        "schema":"studiosat.library-index.v2.1",
...
def build_queue(index):
    ...
    queue={
        "schema":"studiosat.effective-queue.v2.1",
        "generated_at_utc":datetime.now(timezone.utc).isoformat(),
...
class Playout:
    ...
    def desired(self):
        ...
        cur=q.get("current")
        try:
            _pb,_raw,_path,age=parse_playback()
        except Exception:
            age=999999.0
    ...
    def write_state(self):
        ...
        atomic_json(STATE,{
            "schema":"studiosat.playout-v5.1.state.v1",
